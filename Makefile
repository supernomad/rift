...
compile-bin.%:
	...
	@bash ./dist/bin/compile.sh "$(BUILD)" "$(target_$*)" "$(DOCKER)"
...
docker:
	...
	@docker buildx build \
		...
		--tag ghcr.io/supernomad/riftd:$(HASH) \
		--build-arg BUILD=release \
		--file ./dist/docker/riftd/Dockerfile \
		--push \
		.
...
promote:
	...
	@docker buildx build \
		--platform linux/arm64,linux/amd64 \
		--tag ghcr.io/supernomad/riftd:$(TAG) \
		--build-arg HASH=$(HASH) \
		--file ./dist/docker/riftd/Dockerfile.promote \
		--push \
		.
...
coverage:
	...
	@cargo +nightly llvm-cov --ignore-filename-regex '(rift|riftd)\.rs'

coverage-ci:
	@bash ./dist/bin/print.sh "Running tests with coverage"
	@cargo +nightly llvm-cov --ignore-filename-regex '(rift|riftd)\.rs' --hide-instantiations --lcov --output-path coverage.lcov

coverage-html:
	@bash ./dist/bin/print.sh "Running tests with coverage"
	@cargo +nightly llvm-cov --ignore-filename-regex '(rift|riftd)\.rs' --hide-instantiations --html
